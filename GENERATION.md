...
## 与 AutoCraft 0.6.0 配合

AutoCraft 对明确截断、完整响应的 JSON 格式错误或空正文最多恢复一次，并计入原来的总尝试数（最多 3 次）。截断恢复会显式请求较高预算；一旦使用恢复预算，后续结构修正仍使用该受限预算。其他模型错误、拒绝、网络失败和未完成的流不会自动重放。
